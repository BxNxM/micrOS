...
- Firmware/runtime behavior -> edit `micrOS/source/`
- Built-in web UI source -> edit `micrOS/source/web/`
- Host tooling/build/deploy behavior -> edit `toolkit/`, `devToolKit.py`, or `env/`
- Generated deploy/sim output -> generated by tools under `toolkit/workspace/`; do not hand-edit
...
Treat all `toolkit/workspace/*` paths as derived output. Do not manually edit,
format, minify, regenerate, or patch files in this tree as part of a source
change. If a generated artifact is wrong, fix the generator or source input,
then let the normal toolkit command recreate the artifact.

Examples:

- Change firmware code in `micrOS/source/*.py`, not `toolkit/workspace/precompiled/*.mpy`
- Change runtime web assets in `micrOS/source/web/*`, not `toolkit/workspace/precompiled/web/*`
- Change simulator shims in `toolkit/simulator_lib/*`, not `toolkit/workspace/simulator/*`
- Change generation behavior in `toolkit/DevEnvCompile.py`, not the generated files it emits
...
- Never patch `toolkit/workspace/*` files when the real fix belongs in source or generator code
...
Never hand-edit precompiled artifacts, including copied web resources under
`toolkit/workspace/precompiled/web/`. These files are generated by the
cross-compile/precompile pipeline and may be overwritten at any time.

If behavior changes, update the authoritative source in `micrOS/source/`.
If the artifact generation itself is wrong, update the build/precompile code in
`toolkit/`, then run the appropriate toolkit command to regenerate outputs.
Generated output diffs should normally be excluded from source changes unless
the maintainer explicitly asks to refresh committed artifacts.
