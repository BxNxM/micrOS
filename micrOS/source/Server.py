...
class WebCli(Client, WebEngine):
    ...
    def __init__(self, reader, writer):
        Client.__init__(self, reader, writer, r_size=WebEngine.MTU_SIZE)
        WebEngine.__init__(self, version=Shell.MICROS_VERSION)

    async def run_web(self):
        # ONLY CALLED WHEN webui == True (real WebEngine is loaded properly)
        Manager().task_msg('server', ','.join(list(Client.ACTIVE_CLIS)))
    ...
    async def run_server(self):
        ...
        if cfgget('webui'):
            try:
                ...
                Client.console(f"- HTTP server memory error: {e}")
