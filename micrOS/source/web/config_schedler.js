// Loaded on demand by config.js when the Scheduler menu opens.
function renderSchedulerSection() {
  renderConfigFields(filterConfig('Scheduler'), 'Scheduler', (data, container) => {
    Object.entries(data).forEach(([key, value]) => {
      if (key === 'crontasks') renderCrontaskField(container, key, value);
      else renderField(container, key, value);
    });
  });
}

const scheduleTags = ['sunrise', 'sunset'];
const scheduleDays = [
  {id: 0, label: 'Mon'}, {id: 1, label: 'Tue'}, {id: 2, label: 'Wed'},
  {id: 3, label: 'Thu'}, {id: 4, label: 'Fri'}, {id: 5, label: 'Sat'},
  {id: 6, label: 'Sun'},
];
const scheduleDefaultTime = {hour: 9, minute: 10};

function updateUnsavedScheduleFields() {
  const blocks = document.querySelectorAll('#configFields [data-type="crontask"] [data-block-index]');
  if (!blocks.length) return;
  const savedBlocks = parseCrontasks(configData.crontasks);
  const advanced = crontaskSeparator(configData.crontasks) === ';;';
  const pending = changedValues.hasOwnProperty('crontasks');
  const normalize = (field, value) => field.type === 'number' && value !== '' ? String(Number(value)) : String(value);
  const mark = (field, saved) => {
    const value = field.tagName === 'BUTTON' ? field.getAttribute('aria-pressed') : field.value.trim();
    field.classList.toggle('config-unsaved', pending && normalize(field, value) !== normalize(field, saved));
  };
  blocks.forEach((block, index) => {
    if (!pending) block.dataset.blockIndex = index;
    const parts = (savedBlocks[Number(block.dataset.blockIndex)] || '').split('!');
    const timestamp = defaultScheduleTimestamp(parts[0]);
    const sun = timestamp.match(/^(sunrise|sunset)([+-]\d+)?$/);
    const time = (sun ? defaultScheduleTimestamp('') : timestamp).split(':');
    const days = expandScheduleDays(time[0]);
    mark(block.querySelector('input[data-field="timestamp"]'), timestamp);
    ['hour', 'minute'].forEach((part, index) => {
      const value = (time[index + 1] || '').trim();
      mark(block.querySelector(`[data-time-${part}]`), value === '*' ? '' : value);
    });
    mark(block.querySelector('[data-sun-tag]'), sun ? sun[1] : 'sunrise');
    mark(block.querySelector('[data-sun-offset]'), sun ? Number(sun[2] || 0) : '');
    block.querySelectorAll('[data-day]').forEach(button => mark(button, days.includes(Number(button.dataset.day))));
    block.querySelectorAll('[data-schedule-mode]').forEach(button => mark(button, button.dataset.scheduleMode === (sun ? 'tag' : 'time')));
    const functions = parseCrontaskFunctions(parts.slice(1).join('!'), advanced);
    block.querySelectorAll('input[data-function-input]').forEach((input, index) => mark(input, functions[index] || ''));
  });
}
...
function renderCrontaskField(container, key, value) {
  ...
  // Keep fieldsets in block flow: Safari mis-sizes legends in flex columns.
  ...
  blocks.forEach((block, idx) => {
    createCrontaskBlock(blockContainer, key, block, idx);
  });

  // Add button for new block
  const addBlockButton = makeButton('+ Add Schedule', () => {
    createCrontaskBlock(blockContainer, key, '', blockContainer.querySelectorAll('[data-block-index]').length);
...
function createCrontaskBlock(container, key, block, blockIdx) {
  const blockWrapper = createConfigFieldset(`Schedule ${blockIdx + 1}`, 'schedule-card');
  ...
  fnWrapper.className = 'schedule-functions';
  const fnLabel = textElement('label', 'Functions', 'schedule-functions-title');
  ...
  const addFnButton = makeButton('+', null, 'schedule-function-add config-add-button');
  addFnButton.setAttribute('aria-label', 'Add function');
  addFnButton.onclick = () => {
...
function createFunctionRow(container, key, blockWrapper, value, idx, totalCount, addButton) {
  ...
  row.className = 'function-row schedule-function-row';
  ...
  if (fnInputs.length > 0 || totalCount > 1 || totalCount === -1) {
    const delButton = makeButton('−', () => {
      ...
      updateRowAddButtonState(container, addButton, 'input[data-function-input]');
      updateCrontaskTrack(crontaskRoot(blockWrapper), key);
    }, 'schedule-function-remove config-remove-button');
    delButton.setAttribute('aria-label', 'Remove function');
...
function renderScheduleTimeControls(wrapper, tsInput, blockWrapper, key) {
  ...
  const icon = textElement('span', '', 'schedule-icon');
  const summary = textElement('div', '', 'schedule-summary');
  ...
  clockInputs.append(hourInput, textElement('span', ':', 'schedule-clock-separator'), minuteInput);
  timePanel.appendChild(clockInputs);
  const dayRow = textElement('div', '', 'schedule-time-row schedule-day-row');
  scheduleDays.forEach(({id, label}) => {
    const button = makeButton(label, () => {
      button.classList.toggle('selected');
      writeScheduleTime(editor, tsInput, blockWrapper, key, false);
    }, 'schedule-day-button');
    button.dataset.day = id;
...
function scheduleDaysLabel(days) {
  ...
  const sameDays = (expected) => orderedDays.length === expected.length && expected.every((day, index) => orderedDays[index] === day);
  if (sameDays([0, 1, 2, 3, 4, 5, 6])) return 'Every day';
  if (sameDays([0, 1, 2, 3, 4])) return 'Weekdays';
  if (sameDays([5, 6])) return 'Weekend';
  return orderedDays.length ? orderedDays.map(day => scheduleDays[day].label).join(', ') : 'No days selected';
}

function adjustSunOffset(editor, tsInput, blockWrapper, key, direction) {
  const offset = editor.querySelector('[data-sun-offset]');
  offset.value = String((Number(offset.value) || 0) + direction);
...
function updateCrontaskTrack(container, key) {
  ...
  const result = joinCrontasks(blockStrings) || 'n/a';
