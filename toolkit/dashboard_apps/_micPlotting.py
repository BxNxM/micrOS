...
# Note: this script will not run when started from devtoolkit
# because Matplotlib does not support rendering GUI elements
# when not running from the main thread
...
def plotting(frames, window_size, repeat):
    ...
    def update(i):
        ...
        raw_sample = 10     # on device sampling average (TODO: dynamic!)
        if len(x):
