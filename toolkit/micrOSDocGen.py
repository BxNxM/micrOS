...
class MicrOSDocGen:
    MICROS_REPO_BLOB_URL = "https://github.com/BxNxM/micrOS/blob/master"
    MICROS_PACKAGES_REPO_BLOB_URL = "https://github.com/BxNxM/micrOSPackages/blob/main"

    def init_doc_gen_paths(self):
        self.sfuncman_output_path = os.path.join(MYPATH, "../micrOS/client/sfuncman/")
        self.packages_dir_path = os.path.join(MYPATH, "../micrOS/packages")

    @staticmethod
    def _load_module_name_from_path(path):
        file_name = os.path.basename(str(path))
        if file_name.startswith("LM_") and file_name.endswith(".py"):
            return file_name.replace("LM_", "", 1).rsplit(".", 1)[0]
        return None

    @classmethod
    def _builtin_lm_image_url(cls, module_name):
        return f"{cls.MICROS_REPO_BLOB_URL}/media/lms/{module_name}.png?raw=true"

    @classmethod
    def _package_lm_image_url(cls, package_name, module_name):
        return f"{cls.MICROS_PACKAGES_REPO_BLOB_URL}/{package_name}/media/{module_name}.png?raw=true"
    ...
    def _package_module_image_urls(self):
        """
        Map package Load Modules to their package-owned media assets.
        Package image names follow the public LM name without the LM_ prefix.
        """
        module_images = {}
        if not os.path.isdir(self.packages_dir_path):
            return module_images

        for package_name in sorted(os.listdir(self.packages_dir_path)):
            package_path = os.path.join(self.packages_dir_path, package_name)
            ...
            try:
                ...
                self.console(f"[DOC-GEN] Skip package image lookup: {package_json_path}: {e}", state="WARN")
                continue

            for url_entry in package_config.get("urls", []):
                if not isinstance(url_entry, (list, tuple)) or len(url_entry) != 2:
                    continue
                target, source = url_entry
                module_name = self._load_module_name_from_path(target) or self._load_module_name_from_path(source)
                if module_name is None:
                    continue
                module_images[module_name] = self._package_lm_image_url(package_name, module_name)
        return module_images
    ...
    def _build_doc_load_module_structure(self):
        ...
        package_module_images = self._package_module_image_urls()
        ...
        for LM in modules_to_doc:
            ...
            try:
                ...
                module_function_dict[module_name][
                    'img'] = package_module_images.get(module_name, self._builtin_lm_image_url(module_name))
            except Exception as e:
    ...
    def LM_functions_static_dump_gen(self):
        ...
        hardcoded_manual = {"task": {"list": {"doc": "list micrOS tasks by taskID", "param(s)": ""},
                                     ...
                                     "img": self._builtin_lm_image_url("tasks")}
                            }
        ...
        html_tables = self._my_json_to_html(
        ...
        html_tables_ext = self._my_json_to_html(
            module_function_dict_html_ext,
