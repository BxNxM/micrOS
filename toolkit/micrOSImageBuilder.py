...
def manifest_text(source_dir=MICROS_SOURCE):
    """Create the stock ESP32 plus micrOS frozen-module manifest."""
    source_dir = Path(source_dir).resolve()
    files = core_source_files(source_dir)
    files.remove("urequests.py")
    file_lines = "\n".join('        "{}",'.format(name) for name in files)
    return """# Generated by toolkit/micrOSImageBuilder.py; do not edit.
# Use MicroPython's own ESP32 manifest as the stock base so its library set
# stays exactly aligned with the pinned MicroPython release. The override
# library supplies micrOS's API-compatible urequests implementation when the
# stock networking bundle requires that module.
add_library("micros-overrides", "micros-manifest-packages", prepend=True)
include("$(PORT_DIR)/boards/manifest.py")

# Freeze direct micrOS/source/*.py core files. urequests.py is already selected
# through the stock manifest's dependency graph and must not be added twice.
freeze(
    {source_dir!r},
    (
{file_lines}
...
def write_manifest_overrides(output_dir, source_dir=MICROS_SOURCE):
    """Provide micrOS implementations for stock manifest package names."""
    package_dir = Path(output_dir) / "micros-manifest-packages" / "urequests"
    package_dir.mkdir(parents=True, exist_ok=True)
    override_path = package_dir / "manifest.py"
    override_path.write_text(
        """# Generated by toolkit/micrOSImageBuilder.py; do not edit.
metadata(description="micrOS HTTP client compatibility provider")
module("urequests.py", base_path={!r}, opt=3)
""".format(str(Path(source_dir).resolve())),
        encoding="utf-8",
    )
...
def write_manifest(output_dir, source_dir=MICROS_SOURCE):
    ...
    write_manifest_overrides(output_dir, source_dir)
